...
| lives in | `theories/<slug>/` | `studies/<date>-<slug>/` |
...
The shared contracts in [`../tickets/new-theory/README.md`](../tickets/new-theory/README.md)
...
## Tickets about a study live in the study

```bash
python -m tools.cli tickets new --lane study \
    --study 2026-08-29-series-bias-mining --slug <slug> \
    --title "<one line>" --body "<what to do>" --session <you>
```

Same rule as a theory, for the same reason: a study folder holds its
question, its scripts, its data and its verdict, and queued work against
it is part of that. Name the folder exactly, dated prefix and all — a
typo is refused rather than creating a phantom directory.
...
study reaches that point, say so in its `STUDY.md`, keep its status
honest (`collecting`, not `complete`), and expect the floor to report it
as in flight until it is finished.
