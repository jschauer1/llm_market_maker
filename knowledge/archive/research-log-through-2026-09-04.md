...
## 2026-09-04 — shared Codex/Claude workflows and queue consistency

**Did:** Consolidated the former CLAUDE.md research policy into
`docs/RESEARCH_GUIDE.md`, with a compact `AGENTS.md` bootstrap shared by both
applications. Canonical workflows now live in `.agents/skills/`; generated
Claude discovery entrypoints are checked by `python -m tools.agent_setup
--check`. Runtime adapters describe actual native agent operations and capacity
limits. No prompt semantics, theory versions, or historical ledger records
were changed.

Judgment provenance accepts the actual model, supplied effort, search setting,
and exact rendered prompt for every stage and batch. Legacy single-stage calls
remain supported; new explicit backtest executions must state search is off.
Omitted idea sources use `agent` for new ideas and preserve existing historical
attribution. Ticket citation protection now reads both skill trees.

**Learned:** Independent review caught a duplicate-stage restriction that would
have lost batch-specific prompts, a stale skill example, and an idea upsert
that would have overwritten its original provider. Regression tests cover the
corrected behavior. The current host has four global agent slots; a full
three-worker fleet leaves no capacity for nested judges. Claude Code is not
installed on this host, so its entrypoints and workflow were reviewed without
an end-to-end Claude session.

**Returned to general improvements:** The orientation queue now applies the
same supersession identity as promotion, removing an older endorsement only
when the current version actually re-decided that position. Replay,
experiment, opposite-side, and future-version rows cannot incorrectly suppress
it. Rows and total now come from one query. Tests use private databases.

**Next:** `tickets/maintenance/completed/2026-09-04-apply-evidence-eligibility-consistently.md`
records the mismatch between the stated evidence policy and the score/bucket
consumers. It needs targeted eligibility tests before any score recomputation.

**Validation:** Final offline suite: 1,505 passed, 1 skipped, 4 network tests
deselected. Discovery synchronization and whitespace checks are clean; the
independent re-review found no remaining blocking issues in these changes.


---

## 2026-09-04 — consistent evidence and recoverable agent work

**Did:** Applied one production eligibility rule to scores, measured buckets,
slices and settlement-day reports. Valid tier A/B backtests count fully and can
clear existing gates without live outcomes. Excluded replay rows stay available
through raw observations, with counts for tier C, absent tiers/registrations and
wrong theory/version registrations. CLI reports now use one requested mode/run
scope throughout. No historical tier metadata or live derived score was changed.

Added OS-released ownership around actual collector load/mutate/save cycles,
unique flushed atomic-write temporary files, and recoverable ticket transitions
with stable per-ticket locks, exact retry receipts and no-replace publication.
The three resolved maintenance tickets moved to completed through the API.

Shared position facts now keep ledger, promotion and orientation in agreement
about basket settlement and current-version supersession. New judgment batches
persist exact requests and original run state separately from blind judge input,
so a restarted operator can reuse completed work. Native dispatch and historical
replay remain with their runtime and theory owners.

**Agent efficiency:** Both Codex and Claude now load universal constraints plus
the complete guide sections selected by the task map. Every guide section has
an owned routing rule; historical rationale remains in the guide. Canonical
workflow examples point to real prepare/complete/resume APIs. Sol handled the
larger implementations and evidence review, Luna handled ticket transitions,
and Astra reviewed concurrency and final integration.

**Review findings addressed:** Concurrent destination clobbering, prefix-matched
retry receipts, a source-note race, a tier-C diagnostic undercount, the Claude
shim still demanding the whole guide, and restart fidelity for board/payload/time.
The judgment recovery review also required persisting request settings before
native dispatch, so stranded output can be ingested with truthful provenance.

**Validation:** Final complete offline suite: 1,565 passed, 4 network tests
deselected. Discovery synchronization and whitespace checks passed. Every
independent review finding is resolved; two fresh processes recovered two
judgment batches with original inputs and correct provenance, ingesting the
stranded result without another model call. Windows concurrency was exercised;
Claude workflow/discovery was checked statically because its CLI is absent.
Changes remain uncommitted; live research data and saved score caches were
not recomputed.
