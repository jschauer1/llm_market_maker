...
status: done
closed: 2026-09-02
resolution: KILLED, 2026-09-01 by fleet-w2-g2, on the criterion this ticket itself nominated as the likely killer and said to test first. Evidence and reproducible script: tickets/new-theory/evidence/2026-09-01-aggregation-gap-probe/RESULT.md. TWO INDEPENDENT KILLS. (1) STRUCTURAL: this ticket's own scope limit -- 'E[wins] is only recoverable where the strike ladder SPANS THE FULL SUPPORT... or it will manufacture enormous fake gaps' -- applies to its own headline House case and was not applied there. KXHOUSEWINSTATE carries exactly ONE 'more than k' strike per state plus a few 'exactly k', so E[Dem seats] is BOUNDED not determined; bound widths are 0.06-0.40 seats against claimed gaps of +0.21 to +0.40, and in SC the width equals the gap. So the exact conservation law does not exist for this family and the House instance is a relative-value bet on which quote is wrong, not an arbitrage -- which triggers this ticket's own third kill criterion, whose outcomes do not settle until 2027-11-03. (2) ECONOMIC: the gap is positive at mid in 5 of 5 states (mean +0.196) and NEGATIVE at worst-case executable quotes in 5 of 5 (mean -0.240), -0.313 after fees. The line that needs no adjudication: this ticket's own most favourable worst-case gap is +0.073 seats, and the measured fee cost of the basket that captures it is /usr/bin/bash.073 -- the same number, so fees eat 100% of the spec's own edge before a 14-month capital lockup, and the fee figure is a LOWER bound because replicating a seat count needs k contracts per 'exactly k' leg rather than one. THIS IS RULE 0f FOR THE EIGHTH TIME, same shape as combo-vs-leg: a real mid gap sitting entirely inside the spread. WHAT THE TICKET GOT RIGHT AND SHOULD BE KEPT: its NFL reporting was honest and correct (bid/ask band [264.03, 284.47] straddling the 272 conservation constant, 'a sign, not a violation'), and NFL is the one family here with a genuinely complete ladder -- so it is a clean negative on the STRONGEST available form of the cross-event test and belongs in rule 0's evidence list beside combo-vs-leg. Idea 30 recorded dead with those facts plus two traps: KXHOUSEWINSTATE partitions do not always sum to 1 (FL 0.115, NC 0.880), and Kalshi lists incomplete delegations (8 of Michigan's 13 districts), so any screen would need a completeness source outside Kalshi.
---
