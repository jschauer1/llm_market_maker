...
status: done
closed: 2026-09-01
resolution: Split: the atomic-write half is done, the lock half is re-filed as maintenance/collector-write-lock. SHIPPED tools/atomic_write.py -- write_json/write_text write a sibling .tmp then os.replace, retrying a transient OSError, so a reader always sees the whole old file or the whole new one and a OneDrive/AV lock costs a retry instead of a run. On persistent failure the destination keeps its previous contents and a RuntimeError names the path; the .tmp is cleaned up. Elevated from theories/deadline_drift/collect_settled.py::_save, which proved the shape under a real failing walk, once the other callers appeared -- tools/README.md's more-than-one-caller criterion. MIGRATED all four whole-file writers, local copy deleted per the elevation rule: deadline_drift/collect_settled.py::_save (now delegates), deadline_drift/population.py, insider_bias/backfill_history.py, calibration_harvest/collect.py. Registered in toolkit GROUPS so 'cli tools' lists it; dead 'import os' removed from collect_settled.py. 7 tests in tests/test_atomic_write.py, written before the module existed; the load-bearing one asserts a failed write NEVER truncates the existing file, which is what a plain write_text cannot promise -- demonstrated: an open('w') that dies after opening leaves the file empty, and ~60 days of Kalshi settled history is unrecoverable upstream. DOCUMENTED in tools/README.md, beside the record-while-you-collect rule: use atomic_write not Path.write_text, plus the two operational facts this cost a session to learn -- Git Bash 'ps -ef' shows no arguments so 'grep collect_settled' returns zero while it runs (use Get-CimInstance Win32_Process with CommandLine), and stopping a background task stops the shell, not the detached child. NOT FIXED: the lost-update race between two concurrent collectors, which atomicity cannot address and which is now arguably harder to spot because the file is always well-formed. See the new ticket.
---
