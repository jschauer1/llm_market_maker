...
status: done
closed: 2026-09-03
resolution: DONE, both halves, as GUARDS rather than as a move -- series-bias-mining is in flight in the study lane and must not be moved by this session. (1) GITIGNORE: three tests in test_conventions.py -- no study .gitignore entry may name a STATE directory (the form that orphaned parlay-markup), every study entry must still match something (a dead entry protects nothing while reading as if it does), and any study data dir over 10MB must be ignored per git check-ignore. The predicate is unit-pinned against the verbatim pre-fix line, because the entries were already rewritten as globs, so the repo-level test alone would only prove a predicate returned False. Deliberately scoped to STUDY dirs: whether the 10MB rule binds a theory's perishable capture is the open question in 2026-09-03-perishable-capture-dirs, and a test must not silently decide it. (2) CITATIONS: test_every_repo_path_named_in_code_resolves closes the .py gap the ticket names (the docs test only reads Markdown), and advance() now REFUSES a move that would orphan an executable citation, listing them. Verified non-vacuous: 46 literals scanned, both at-risk modules covered plus one in tools/collectors.py the ticket did not know about. HEADS-UP FOR WHOEVER ADVANCES series-bias-mining: the guard will refuse until these 6 are repointed -- no_side_premium/studies/answer/2026-09-01-liquidity-filtered-side-split/completion_checks.py, .../2026-09-01-side-split-60day-obs/measure.py, and the study's own collect.py, mine.py, pass3.py, pass4.py, which self-reference 'investigation/' and break on their own move. Suite 1438 passed.
---
