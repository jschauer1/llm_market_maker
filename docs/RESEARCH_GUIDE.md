# Market Edge Finder — shared research guide

This is the complete research policy for both Codex and Claude Code.
Load the task-relevant sections using `docs/agents/policy-map.md`, with the
runtime adapter selected by `AGENTS.md`. Historical
references to CLAUDE.md refer to this policy at their recorded revision.
...
- **`supervise`** — a fleet. A supervisor keeps a capacity-derived target of
  up to three `go` research workers active, after reserving one advertised
  global slot for required worker-created judgment and counting other active
  consumers. It judges what workers bring back and is the only thing that
  writes to git. It claims no lane, opens no notebook, and never substitutes
  for the required judge.
...
(`RUNBOOK.md`); historical `NOTES.md`, new `notes/`, and scoped `learnings/`;
a campaign's own inputs and
...
**Research context is read through summaries.** Each theory's
`<theory registry path>/learnings/README.md` selects short, scoped lesson cards that link to original
evidence. Historical `NOTES.md` remains available; new exploratory narrative
uses one question per file in `notes/`. `THEORY.md` remains the specification
and changes when the claim, procedure, or status changes. The complete writing,
compression, and retrieval contract is in `docs/agents/research-memory.md`.
...
initialized inside one theory — handed AGENTS.md, this guide, the skills, and the
...
- **The supervisor's contract: every fact the supervisor needs in order to
  ...
  or `RESEARCH_LOG.md`, reached through summary maps. A theory whose true status is discoverable only by
...
Measured bucket transfer still obeys binary payout bounds: bound the
inferred probability to `[0, 1]` and derive gross/net from that same value.
For both calibration and live re-quoting, a price and its estimate must come
from one recorded decision. [Attempt selection](../knowledge/lessons/keep-estimates-on-one-attempt.md)
defines which one; a position rollup is not an interchangeable observation.
...
Start at `knowledge/README.md`: question summaries lead to topic/theory maps,
then individual lessons, then original evidence. Read only the relevant
branch. Before saving a finding, read `docs/agents/research-memory.md` for the
required lesson fields, word budgets, consolidation, and link maintenance.
Routine sessions need no global log entry. Current status comes from the
existing CLI, tickets, and theory specification, not copied map snapshots.
...
`RETIRED.md`, `THEORY.md`, `NOTES.md`, `RESULTS.md`, Markdown `learnings/`
and `notes/` — plus the theory's own
...
Changing only the model/provider while following the same written procedure
does not bump the version; apply the idealized-judge assumption under
"Record what judged, and what you asked it" and aim for comparable intelligence.
...
- **B** — outcome judgment, markets resolved after the model's knowledge
  ...
  **do not discount a tier B result a second time for being tier B.** What
  ...
  verdicts move). Investigate and disclose those concerns when validating
  whether a replay qualifies. A qualifying tier B run keeps full production
  weight; these concerns are not an extra ranking or probability discount.
...
**Match intelligence levels across runtimes.** When a theory has a benchmark,
use its judging model as the capability reference; otherwise use the role's
tier above. Choose the closest available capability and appropriate reasoning
effort. `insider_judgment`'s judged benchmark used Sonnet, so aim for Sonnet-class
capability there. Use the active inventory and record the choice; exact vendor
equivalence is not assumed. If no close match exists, use the best available
model and disclose the mismatch. Imperfect equivalence alone does not impose
...
Research runs through the user's Codex or Claude Code subscription.
There are no API keys in this repo, and none should be added. Use the
runtime adapter selected by AGENTS.md for available tools and models.

## Record what judged, and what you asked it

**The written procedure defines an idealized judge** (user ruling 2026-09-04).
Assume models following the same logical rules make the same decisions, even
though actual classifications may differ. Aim for comparable intelligence and
reasoning levels across runtimes. A model/provider switch alone shares the
theory's applicable bucket calibration, scores, and slice evidence; it requires
no validation experiment, version bump, evidence reset, or extra discount.
This is an operating assumption, not a measured claim of model equivalence.

**The runtime is not the judging model.** Record the actual requested model
identifier (or a clearly labelled unresolved alias), effort, search setting,
and each batch's rendered prompt so differences can be investigated later.
Never relabel historical model names. Changes to the written judging rules,
bucket definitions, population, or information access follow normal procedure
versioning. Model comparisons are optional research, not a production gate.
A newly executed historical replay still needs its own verified knowledge
cutoff and point-in-time restrictions; sharing calibration does not transfer
another model's cutoff or make contaminated evidence eligible.
...
The prompt's logical rules are part of the decision procedure, like a
threshold; changes to those rules bump the version. Model identity is execution
provenance under the idealized-judge assumption. Saving both lets a later
investigation distinguish a changed procedure from variation between models
...
python -m tools.cli provenance record --theory <slug> --version <n> \
    --run <run_id> --stage analysis --model "<actual-requested-model>" \
...
A fully mechanical theory keeps `uses_llm_judgment=False` and needs no model
execution receipts. It may voluntarily declare a deciding code artifact in
`prompts`; `finish()` then records it in `judgment_runs` as
`none (deterministic)` with web search disabled. That row documents code,
not an LLM dispatch. Without such a declaration, no provenance row is needed.
...
2. **`RESEARCH_LOG.md`** — bounded consequential changes; older entries move
   intact to the linked archive under the research-memory policy.
...
6. **Knowledge navigation** — add or update a summary link to your own finding
   in `knowledge/`; ownerless studies and shared operations may keep a lesson
   card there. This permits distillation and navigation, not depositing raw
   data or duplicating a theory's lesson outside its owner.
...
- **Historical `NOTES.md` and new `notes/`** hold owner-local investigation
  narrative. Reusable conclusions are consolidated in `learnings/`, selected
  through its summary map. Any session may retrieve relevant source sections.
- **`RESEARCH_LOG.md`** records consequential changes with a short explanation
  and canonical source link. It is not a task diary. Orient with the CLI and
  `knowledge/README.md`; consult the linked archive for historical rationale.
