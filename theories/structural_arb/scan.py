...
_KNOWN_CLOSURE = {
    "greater": (False, False),           # (lo_closed, hi_closed); hi at +inf
...
def yes_set(market: Market) -> YesSet | None:
    ...
    A one-sided type carrying BOTH bounds is refused: live example
    KXSTARSHIPSPACE-26-8.0 declares strike_type='less' with
    floor=cap=8 while its title reads "exactly 8" — its YES set is the
    point {8}, and believing the type field would have manufactured a
    'riskless' pair that loses whenever the outcome lands between the
    strikes. Metadata that contradicts itself proves nothing."""
    raw = market.raw or {}
...
def underlying_key(market: Market) -> str | None:
    ...
    proofs are only valid within one underlying, so candidates group by
    this key and None never groups.
