...
**Input:** `{input_path}` — {n_events} Kalshi events, {n_markets} individual markets. Today is **{today}**.
...
Write your verdicts to `{output_path}` as a JSON array, one object per **event** ({n_events} objects), each:
