...
- `<theory>/learnings/README.md` — bounded lesson menu; `NOTES.md` explains the archive
  convention, with new narrative in `notes/YYYY-MM-DD-<question>.md`
...
```bash
python -m tools.cli tickets new --lane new-theory --slug <slug> --title "<one line>" --body "<the thesis and why it might work>" --session <you>
```
...
whether it ran, and what evidence it has. Consolidate reusable lessons and the
theory's summary map under `docs/agents/research-memory.md`; log only a
consequential cross-session change, with a link to the theory.
