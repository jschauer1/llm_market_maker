...
Read `AGENTS.md` and the runtime adapter it selects before using host agent,
model, wait, stop, scheduler, or shell operations. The adapter's current tool
inventory overrides examples and old tool names.
...
Then inspect live agents using the selected runtime adapter, so you know who is
actually live rather than who left a stale claim. If the host has no roster
operation, use only task ids whose status the host can verify; never infer a
live peer from a lane claim alone.
...
- Start with `knowledge/README.md` and the relevant summary branches,
  including other theories when useful. Open matching lessons and specific
  source sections; read complete runbooks when executing them. Nothing here
  is private, and a map is a menu rather than a demand to read every leaf.
...
- **What work did the last session leave?** Open owner tickets carry the next
  action; relevant lesson cards explain what prior work established and when
  it is worth revisiting. The global log carries consequential changes.
...
### Backlog pressure is not optional to notice
...
guarantee, not a discretionary lane — if the floor is due and unheld,
...
```bash
python -m tools.cli lane claim --lane <lane> --session <your name> [--focus <theory>]
```
...
```bash
python -m tools.cli tickets new --lane maintenance --slug <slug> --title "<one line>" --body "<what to do, for someone who was not here>" --session <you>
...
1. **A structural surface answers it** — the promotion key, a runbook, a
   skill, `cli rulings list`, AGENTS.md → take that answer and cite it.
...
- **Close with durable evidence and concise knowledge.** Follow
  `docs/agents/research-memory.md`: consolidate a lesson if one was learned,
  file open work in tickets, and log only consequential cross-session changes.
