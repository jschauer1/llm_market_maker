...
Start with `knowledge/README.md` and the matching topic/theory summaries. Read
the relevant cards and sources, then perform the idea-registry checks below.
No matching card does not mean the hypothesis has never been tried.

Before any other work:

<!-- rule: search-the-registry (moved from AGENTS.md § Research memory, 2026-08-29) -->
...
```bash
python -m tools.cli ideas record <slug> "<title>" --description "<the hypothesis in a sentence or two>" --source agent
```
...
those programmatically unless it is absolutely impossible; docs/RESEARCH_GUIDE.md's
...
Create `theories/<slug>/` with the selected runtime's filesystem operation.
Copy `theories/_TEMPLATE/THEORY.md`, `theories/_TEMPLATE/NOTES.md`, and
`theories/_TEMPLATE/learnings/README.md` into their corresponding paths,
preserving the templates before making theory-specific edits. Then run these
repository commands as separate, single-line commands:
...
Fill in `THEORY.md` completely. New narrative uses dated question files in
`notes/`; reusable findings use scoped cards in `learnings/`, indexed through
its README. Update `knowledge/theories.md` with a summary link to the theory
map and follow `docs/agents/research-memory.md`. Then scaffold the
...
<!-- rule: facts-are-data (moved from AGENTS.md § The theory contract, 2026-08-29) -->
...
<!-- rule: revisit-angle (moved from AGENTS.md § Research memory, 2026-08-29) -->
...
```bash
python -m tools.cli ideas status <slug> dead --what-was-tried "<what you actually did>" --outcome "<why it does not work>" --revisit-angle "<what a different approach would look like, or omit>"
```
