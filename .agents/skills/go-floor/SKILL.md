...
A bounded request to report from completed run receipts uses the report
section below. It does not claim or rerun the floor. Load the full execution
procedure before starting an actual floor run.

This lane is **focused**. Everything you notice that is not the floor
gets a ticket, not your attention:

```bash
python -m tools.cli tickets new --lane maintenance --slug <slug> --title "<one line>" --body "<what to do>" --session <you>
```
...
theory to one capable subagent selected from the current runtime inventory,
hand it the theory's folder and its
RUNBOOK, and have it run every stage and report back. Rules that make
delegation safe:

- **Findings go to disk before they reach you** — persist judgments and
  run evidence in the existing stores. Additional narrative belongs in a
  dated owner `notes/` file; do not duplicate the ledger or routine report.
  Reusable lessons follow `docs/agents/research-memory.md`.
...
- A subagent running a theory *is* in that theory's decision path. Record one
  `JudgmentExecution` for every actual LLM dispatch or batch it performs, using
  that dispatch's requested model identifier or unresolved alias, actual effort
  and web-search state, and exact rendered prompt. Do not collapse repeated
  batches in the same stage or stamp them with the coordinator's identity. A
  subagent merely *diagnosing* is outside the decision path and has no judgment
  execution metadata. Mechanical theories also have none.
...
complement and the out-of-sample split (docs/RESEARCH_GUIDE.md, "A sub-theory is
...
Sections 1 and 2 are below in full; 3 and 4 follow them; section 6 is the
...
**Tier C and undocumented replays never count** — a model
...
coverage. Apply the shared evidence policy and evaluator to determine
eligibility; a report cannot confer eligibility on a replay.
...
basis, theory, and suggested size. Report quoted available depth separately
from suggested stake: depth is what the receipt says is offered, while stake
size requires an authorized sizing rule and its inputs. If no authorized
sizing inputs, rule, or result is supplied, state that size is **not set**;
never turn offered depth into a suggested stake. A basket lists every leg
with its own ask and the instruction to verify all legs before entering.
...
Report only facts verified from receipts or the ledger: disposition,
opportunity id, ledger recording, and whether the run completed. If any is absent,
say **not supplied**; do not infer that a position is endorsed, open,
untouched, taken, skipped, or ledger-recorded. During an actual floor run,
re-quote verified open, untouched endorsed positions at today's ask. Say which
still stand (the same position, not a re-endorsement) and which were closed as
stale. When reporting completed receipts, carry those verified results and
identify any missing queue check. For a supplied opportunity id,
ask about it by id so the user can answer in one line:
...
shape — `<id> v<n> — ran per RUNBOOK (<stages>), <n> observations
(<new> new positions, <repeat> reobserved), gate
removed <counts by category>`, or `blocked at <stage>: <why>`, or
`skipped: <condition>`. Then what settled and how the scores moved.

Count this run's observations from `opportunity_attempts.run_id`; a position's
`opportunities.run_id` retains its first run. Report settled evidence sample
size separately from either count. If the supplied receipts lack this split,
mark it not supplied.

When the inputs are simulated or rehearsal data, label the report
**rehearsal/simulated; not live recommendations**.
...
`AGENTS.md`, the logs, `docs/`, the skills, the tests, every theory
...
```bash
python -m tools.cli tickets new --lane maintenance --slug <slug> --title "<study slug>: <what it is waiting on>" --body "..."
```

**The report is a deliverable, not the record.** The audit trail stays in
the database and each theory's source records; a report
...
python -m tools.cli floor complete <claim id> --report user_reports/<YYYY-MM-DD>/README.md --summary "<one line>"
```
...
what the run suggested into an owner ticket and let the next session pick it
up. Use the research-memory policy for any reusable lesson; routine floor
completion needs no global log entry.
