...
`get_board(conn)` **without** `force`; open your own `db.connect()`; and save a
durable receipt for every actual LLM dispatch or batch before ingesting its
result. Each receipt contains the prompt stage, exact requested model identifier
when the runtime exposes it (otherwise the requested alias labeled as
unresolved), actual effort when supplied or exposed, actual live web-search
state (`True`, `False`, or `None`), and the exact rendered prompt the judge saw.

Use the prepare/complete/resume procedure in
`docs/agents/judgment-batches.md`. `tools/judgments.py` persists the original
screened run and each batch's exact request before dispatch. Write the bare
blind payload to the prompt's input path; receipts and the price-bearing run
state stay with the operator. Complete each returned batch immediately.

After an interruption, `judgments.load_run_state` restores the original run
without screening again. Ingest any stranded verdict output, dispatch only
incomplete batches, and call `run.attach_completed_batches(receipts).finish()`
when the required batches are complete. Repeated executions of one known stage
remain distinct provenance records. Multi-stage runs specify `verdict_stage`;
all declared stages still need their receipts. This preserves original board,
time, candidates, payload, and actual per-batch execution identity.

Mechanical theories finish without judgment receipts. `judge_model` remains a
legacy single-stage fallback; new judged runs use persistent batches.
...
**Provenance is enforced through the contract.** `theory.prompts` maps each
judging stage to a prompt file in the theory's `prompts/` folder. The normal
`run.apply(verdicts).finish()` path records one provenance row per actual
execution before any opportunity row lands, associates it with that stage's
template path, and hashes the exact rendered prompt. Same-stage executions with
different models or rendered prompts remain distinct; exact duplicate metadata
is idempotent. Missing or unexpected stages fail instead of silently producing
incomplete provenance.

There is no separate manual `provenance record` call on the normal theory path.
The durable dispatch receipts are still required before ingestion so a failed
session cannot erase what was sent or returned. Mechanical theories have no
judgment executions and record no invented execution metadata.

<!-- rule: batch-and-dedupe (moved from AGENTS.md § Subagents — cheap gates, expensive analysis, 2026-08-29) -->
...
<!-- /rule -->
<!-- rule: buckets-from-deep-stage (moved from AGENTS.md § Subagents — cheap gates, expensive analysis, 2026-08-29) -->
...
<!-- rule: judge-blind (moved from AGENTS.md § Never state a probability you introspected, 2026-08-29) -->
...
```bash
python -m tools.cli rank --edge <edge_pts_net> --n <settled_n> --calibration-edge-net <cal_net> --mean-claimed-edge <claimed>
```
