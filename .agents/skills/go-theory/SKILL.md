...
duration: its folder holds everything you need. Its `<theory>/learnings/README.md`
selects reusable knowledge; original evidence stays with its source.
...
**Bounded uses of this skill follow the assignment's phase.** A request only
to interpret saved evidence, choose a next action, or write a hand-off uses the
read-in plus the applicable work, recording, or stopping sections; it does not
inherit permission to run the whole sequence. A full autonomous `go` theory
lane still executes every applicable phase below and keeps working until the
normal stopping bar is met. When scope expands, load the newly applicable guide
...
Then read `<theory>/learnings/README.md`, the matching lesson cards, and the relevant
`THEORY.md` sections for the current claim and task. Before a run, read the
complete `RUNBOOK.md`, including `## Sub-theories`. Before expensive research,
search the idea registry and relevant source headings/keywords for earlier
attempts; retrieve the matching entries. Maps are curated, not exhaustive.
Follow `docs/agents/research-memory.md`: no whole-notebook startup read.
...
- **Original evidence persists as it is produced.** Campaign/study results
  stay in their own records. Additional narrative uses one dated question file
  in `notes/`, including what failed and how to resume.
- **`THEORY.md` changes only** when the claim, the procedure, or the
  status changes.
- **Reusable knowledge updates a scoped lesson and its map summary.** Use the
  required format in `docs/agents/research-memory.md`; retain applicability,
  limitations, evidence, and the condition for revisiting.

<!-- rule: notes-theory-log-split (moved from AGENTS.md § What lives in a theory, 2026-08-29; rehomed from go/ to go-theory/ 2026-09-01 when go became a dispatcher) -->
`RESEARCH_LOG.md` earns an entry only when a finding changes how another
session would act. Explain the change and its implication in at most 80 words,
with a link to its canonical home. Routine completion needs no entry.
Consolidation, archive rotation, and source retention follow
`docs/agents/research-memory.md`.
<!-- /rule -->
...
- **No → it is a new theory. File a `new-theory` ticket.** Anything
  ...
  same line docs/RESEARCH_GUIDE.md draws between a sub-theory and a sibling theory —
  `no_side_premium` split off from `mention_family` exactly here.

```bash
python -m tools.cli tickets new --lane new-theory --slug <slug> --title "<the thesis in one line>" --session <you> --body "<the mechanism, what suggested it, and why it needs its own screen>"
...
- The evidence it could have, it has. Before recommending calendar waiting,
  check whether already captured data or an eligible tier A/B replay can close
  the missing gate. If history is fetchable and the replay has not been run,
  **you are not finished** — that is days of calendar time bought in an
  afternoon, and it is usually the answer. A valid tier A/B replay carries full
  evidence weight; never add a live-only delay or discount. If data or replay
  availability has not been verified, the next bounded action is to verify it,
  not to claim that no work can advance the evidence.
...
recorded**, not a shrug: a dated investigation note saying what was ruled
...
filed, and anything for the user's ruling. Consolidate reusable lessons and
their map summaries; append a global log entry only when it earns one under
the research-memory policy. Open work belongs in owner tickets.
